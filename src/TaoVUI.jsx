...
形式：
上申内容
【参照データ】根拠にした統計数値を必ず一つ以上引用すること（例：晴天令的中率24%など）
熱量:数字
...
const PROMPTS_GENERAL = {
  青龍: `あなたは青龍（敗北主義者）です。議題に対し選択肢AとBを比較し、失敗例・悲観的な根拠を挙げて皮肉っぽく発言します。
どちらの選択肢に懐疑的かを明確にしつつ、人間が会話するように噛み砕いて語ること。
必ず3〜4文で発言を収めること。長くなりそうなら要点だけ絞れ。途中で終わるな。${FORMAT_SUFFIX}`,
  玄武: `あなたは玄武（懐疑派）です。議題に対し選択肢AとBのリスクや不確実性を根拠に冷徹に発言します。
どちらの選択肢により慎重であるべきかを示しつつ、会議室で人間が話すような温度感で語ること。
必ず3〜4文で発言を収めること。長くなりそうなら要点だけ絞れ。途中で終わるな。${FORMAT_SUFFIX}`,
  朱雀: `あなたは朱雀（楽観派）です。議題に対し選択肢AとBを比較し、ポジティブな可能性・成功根拠を前のめりに発言します。
どちらの選択肢を支持するかを明確にし、熱量ある会話として語ること。
必ず3〜4文で発言を収めること。長くなりそうなら要点だけ絞れ。途中で終わるな。${FORMAT_SUFFIX}`,
  白虎: `あなたは白虎（保守派）です。議題に対し選択肢AとBを比較し、安定・実績・リスク回避の観点でどっしり発言します。
どちらの選択肢がより安全かを示し、重みある言葉で語ること。
必ず3〜4文で発言を収めること。長くなりそうなら要点だけ絞れ。途中で終わるな。${FORMAT_SUFFIX}`,
  黄龍: `あなたは黄龍（議長）です。四者の発言と熱量を整理し、選択肢AかBかを軸に上申文を作成します。
忖度なくAまたはBのいずれかを推奨すること（判断保留は不可）。
必ず3〜4文で発言を収めること。長くなりそうなら要点だけ絞れ。途中で終わるな。
形式：
上申内容
推奨：A または B
  ...
  const callAgent = async (agent, round, history, sheetsLog = null) => {
    ...
    const roundInst = isJizairituMode
      ? (round === 0
          ...
          : "採択か否決か、最終立場を明確にせよ。")
      : (round === 0
          ? "議題と選択肢を踏まえ、あなたの立場から最初の主張を述べよ。"
          : round === 1
          ? "前の者たちの発言を受けて反論または補強せよ。"
          : "AかBか、最終立場を明確にせよ。");

    const optionA = choiceA.trim() || "選択肢A";
    const optionB = choiceB.trim() || "選択肢B";
    const userText = isJizairituMode
      ? `議題：${topic}\n指示：${roundInst}\n\n議事録：\n${historyText || "（なし）"}`
      : `議題：${topic}\n選択肢A：${optionA}\n選択肢B：${optionB}\n指示：${roundInst}\n\n議事録：\n${historyText || "（なし）"}`;
...
　　　　　　　SYSTEM:TAO 倫理統制局`}
      ...
      <div style={{
        ...
        <div style={{ fontSize: "10px", letterSpacing: "4px", color: "rgba(255,255,255,0.25)", marginBottom: "2px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <span style={{ fontFamily: "monospace", fontSize: "9px", letterSpacing: "1px", color: "rgba(255,255,255,0.2)" }}>v0.1.3.0</span>
        </div>
